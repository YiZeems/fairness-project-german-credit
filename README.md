...
![Responsible AI, German Credit](img/responsiveAI-german-credit.png)
...
- **Fairness**: audit (DP, EO, Disparate Impact), intersectional audit (gender x age), pre-processing reweighing (Kamiran & Calders 2012), per-group post-processing thresholds (Hardt et al. 2016), `fairlearn ThresholdOptimizer`.
...
V3 is the main result: it improves both performance (AUC 0.785 to 0.790) and equity per attribute (DI age 0.69 to 0.87, above the EEOC threshold) without any in/post-model intervention. The intersectional bias (gender x age, DI = 0.48) remains unresolved.

## Stack

- [`scikit-learn`](https://scikit-learn.org/) for models, metrics, pipelines, permutation importance.
- [`fairlearn`](https://fairlearn.org/) for `MetricFrame`, `demographic_parity_difference`, `equalized_odds_difference`, `ThresholdOptimizer`.
- [`shap`](https://shap.readthedocs.io/) for `LinearExplainer` (exact form for linear models).
...
2. Exploration and sensitive attributes: distributions, default rates per group (age threshold = 25 years).
   - 2.1 Proxy detection (univariate correlation `|r| > 0.15`, multivariate AUC features vers sensitive attribute).
...
8. Comparison: 4 configurations x 3 variants, impossibility theorem.
