...
\large Rapport accompagnant le notebook \texttt{responsiveAI-german-credit\_rendu.ipynb}}}
\author{%
IA708 --- T\'{e}l\'{e}com Paris, 2026 \\[0.4em]
\normalsize \textbf{\'{E}tudiants :} \\
...
\subsection{Comparaison \`{a} des baselines triviaux}

Pour situer la performance du baseline (AUC = 0.785), on la compare \`{a} trois pr\'{e}dicteurs naïfs : un classifieur al\'{e}atoire stratifi\'{e}, une r\`{e}gle "younger $\Rightarrow$ d\'{e}faut" et une r\`{e}gle bas\'{e}e sur \texttt{checking\_status} (la feature la plus importante selon SHAP).
...
\midrule
Al\'{e}atoire stratifi\'{e} (DummyClassifier) & 0.524 \\
R\`{e}gle ``younger $\Rightarrow$ d\'{e}faut''  & 0.581 \\
R\`{e}gle ``checking\_status $<$ 0''  & 0.661 \\
\textbf{R\'{e}gression logistique (baseline)} & \textbf{0.785} \\
\bottomrule
\end{tabular}
\caption{Comparaison \`{a} des baselines triviaux.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{../outputs/13_trivial_baselines.png}
\caption{R\'{e}gression logistique vs trois baselines triviaux.}
\end{figure}

\begin{leconbox}[Le\c{c}on]
La r\'{e}gression logistique apporte un gain de \textbf{+0.124} d'AUC par rapport \`{a} la meilleure r\`{e}gle naïve (\texttt{checking\_status}). Ce gain justifie l'usage du mod\`{e}le ML par rapport \`{a} une heuristique m\'{e}tier simple. Inversement, on note qu'une r\`{e}gle uniquement bas\'{e}e sur l'\^{a}ge atteint d\'{e}j\`{a} 0.58 d'AUC : l'\^{a}ge porte un signal r\'{e}el mais limit\'{e}, et l'utiliser seul serait \`{a} la fois insuffisant et discriminatoire.
\end{leconbox}
...
\midrule
\textbf{AUC ROC} & Invariante au seuil. Mesure la capacit\'{e} \`{a} s\'{e}parer les classes. \\
\textbf{Balanced Acc.} & $\frac{1}{2}(\text{TPR}+\text{TNR})$. Adapt\'{e}e au d\'{e}s\'{e}quilibre (vs accuracy na\"{i}ve). \\
\textbf{Co\^{u}t m\'{e}tier} & $5 \cdot \text{FN} + 1 \cdot \text{FP}$ (matrice UCI : approuver un mauvais cr\'{e}dit co\^{u}te 5$\times$ plus). \\
\bottomrule
\end{tabular}
\end{table}
...
Le baseline n'est pas \'{e}quitable. Pour l'\^{a}ge, le $|\Delta\text{DP}| = 0.125$ et le DI = 0.69 (sous le seuil l\'{e}gal US 0.8). Pour le genre, biais quasi nul apr\`{e}s retrait de \texttt{personal\_status\_sex}. Cela justifie l'\'{e}tape suivante : auditer les sous-groupes intersectionnels avant de mitiger.
\end{attentionbox}

\subsection{Audit intersectionnel : combinaisons genre $\times$ \^{a}ge}

L'audit s\'{e}par\'{e} (genre seul, \^{a}ge seul) peut masquer des biais qui n'apparaissent qu'\`{a} l'intersection. On combine les deux attributs en un attribut \`{a} 4 niveaux et on r\'{e}-applique \texttt{fairness\_summary}. R\'{e}f\'{e}rence : cours \texttt{fairness-ms-1.pdf} (slide sur les biais multivari\'{e}s).
...
male\_younger   & 19 & 0.26 & 0.86 \\
\bottomrule
\end{tabular}
\caption{Audit intersectionnel sur les 4 sous-groupes \texttt{genre\_age}.}
\end{table}

\noindent Synth\`{e}se des \'{e}carts agr\'{e}g\'{e}s vs intersectionnels :
...
\^{A}ge seul   & 0.125 & 0.066 & 0.69 \\
\textbf{Intersection} & \textbf{0.181} & \textbf{0.222} & \textbf{0.59} \\
\bottomrule
\end{tabular}
\caption{Comparaison des \'{e}carts d'\'{e}quit\'{e} : agr\'{e}g\'{e}s vs intersectionnel.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{../outputs/14_intersectional.png}
\caption{Approval rate par sous-groupe (gauche) et comparaison des \'{e}carts agr\'{e}g\'{e}s vs intersectionnel (droite).}
\end{figure}

\begin{leconbox}[Le\c{c}on]
L'intersection r\'{e}v\`{e}le un biais \textbf{1.5$\times$ plus marqu\'{e}} que l'analyse par axe s\'{e}par\'{e}. DI = 0.59 est gravement sous le seuil l\'{e}gal US 0.8. Ce sont les \textbf{jeunes hommes} qui ont le taux d'octroi le plus bas (0.26) sur ce dataset, contrairement \`{a} l'intuition initiale "les jeunes femmes sont les plus d\'{e}favoris\'{e}es". \`{A} interpr\'{e}ter avec prudence vu les petits effectifs (n = 19, 25).
\end{leconbox}
...
Les deux groupes ont des distributions de scores diff\'{e}rentes. En appliquant le m\^{e}me seuil, on s\'{e}lectionnerait des proportions diff\'{e}rentes. L'ajustement de seuil par groupe r\'{e}\'{e}quilibre cela.
...
\subsection{Post-processing officiel : fairlearn ThresholdOptimizer}
\label{sec:fairlearn-eo}

Notre code maison \'{e}galise le taux de s\'{e}lection (proxy de Demographic Parity). La librairie \texttt{fairlearn} fournit l'impl\'{e}mentation officielle de Hardt et al.\ 2016 qui peut imposer plusieurs contraintes (DP, EO, equalized\_odds), via une optimisation lin\'{e}aire des seuils par groupe.

\begin{table}[H]
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Strat\'{e}gie} & \textbf{BalAcc} & $|\Delta\text{DP}|$ & $|\Delta\text{EO}|$ & \textbf{DI} \\
\midrule
Baseline (sans PP)                  & 0.669 & 0.125 & 0.066 & 0.69 \\
PP maison (Demographic Parity)      & 0.644 & 0.060 & 0.135 & 0.86 \\
fairlearn ThresholdOpt (EO)         & 0.596 & 0.060 & \textbf{0.300} & 0.94 \\
\bottomrule
\end{tabular}
\caption{Comparaison de 3 strat\'{e}gies de post-processing sur l'\^{a}ge.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{../outputs/15_fairlearn_postproc.png}
\caption{\'{E}quit\'{e} (gauche) et performance (droite) pour 3 strat\'{e}gies de post-processing.}
\end{figure}

\begin{leconbox}[Le\c{c}on : limite des m\'{e}thodes optimis\'{e}es sur petit dataset]
Sur ce dataset (val = 200, test = 200), \texttt{ThresholdOptimizer} \textbf{d\'{e}grade} $|\Delta\text{EO}|$ (0.066 $\to$ 0.300) au lieu de l'am\'{e}liorer. Le m\'{e}canisme : il randomise les d\'{e}cisions pour atteindre la contrainte, et calibr\'{e} sur 200 lignes, ne g\'{e}n\'{e}ralise pas. Sur datasets plus grands, il am\'{e}liore syst\'{e}matiquement (Hardt et al.\ 2016). \textbf{Conclusion} : sur petit dataset, mieux vaut une heuristique simple (PP maison) qu'une optimisation officielle qui exige plus de donn\'{e}es.
\end{leconbox}
...
\begin{leconbox}[Le\c{c}on]
\textbf{L'IC95\,\% de $|\Delta\text{DP}|$ \^{a}ge s'\'{e}tend de 0.02 \`{a} 0.20.} Le biais empirique de 0.125 est dans cet intervalle, mais l'incertitude est tr\`{e}s large (test de 200) $\Rightarrow$ \textbf{prudence m\'{e}thodologique}. Sur ce dataset, les \'{e}carts d'\'{e}quit\'{e} ne sont pas significatifs \`{a} un niveau pr\'{e}cis.
\end{leconbox}
